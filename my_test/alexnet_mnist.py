...
lc_checkpoint_model = AlexNet().to(device)
...
delta_lora_model = AlexNet_LowRank(w, b, rank = RANK).to(device)
dlora_lc_model = AlexNet_LowRank(w, b, rank = RANK).to(device)
...
# 上一个基线检查点，用于模拟恢复
last_dlora_lc_baseline_checkpoint = AlexNet()
...
for epoch in range(num_epochs):
    for iter, data in enumerate(trainloader):
        ...
        if iter == 0 and epoch == 0:
            ...
            last_dlora_lc_baseline_checkpoint = lazy_restore(dlora_lc_weights, dlora_lc_decomp_weights, bias, AlexNet(), 
            ...
            dlora_lc_model = AlexNet_LowRank(w, b, rank = RANK)
            dlora_lc_optimizer = torch.optim.SGD(dlora_lc_model.parameters(), lr = learning_rate)
        ...
        if iter % 100 == 0 and iter != 0:
            ...
            restored_model = lazy_restore(dlora_lc_weights, dlora_lc_decomp_weights, bias, AlexNet(), 
