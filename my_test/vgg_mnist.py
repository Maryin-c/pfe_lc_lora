...
baseline_model = VGG16().to(device)
lc_checkpoint_model = VGG16().to(device)
...
delta_lora_model = VGG16_LowRank(w, b, rank = RANK).to(device)
dlora_lc_model = VGG16_LowRank(w, b, rank = RANK).to(device)
...
# 上一个基线检查点，用于模拟恢复
last_dlora_lc_baseline_checkpoint = VGG16()
...
for epoch in range(num_epochs):
    for iter, data in enumerate(trainloader):
        ...
        if iter == 0 and epoch == 0:
            ...
            last_dlora_lc_baseline_checkpoint = lazy_restore(dlora_lc_weights, dlora_lc_decomp_weights, bias, VGG16(), 
            ...
            dlora_lc_model = VGG16_LowRank(w, b, rank = RANK)
            dlora_lc_optimizer = torch.optim.SGD(dlora_lc_model.parameters(), lr = learning_rate)
        ...
        if iter % 100 == 0 and iter != 0:
            ...
            restored_model = lazy_restore(dlora_lc_weights, dlora_lc_decomp_weights, bias, VGG16(), 
...
plt.plot(full_accuracy, label = "Default VGG16")
plt.plot(lc_accuracy, label = "LC VGG16")
plt.plot(decomposed_full_accuracy, label = "dLoRA VGG16")
plt.plot(restored_accuracy, label = "dLoRA + LC VGG16")
...
plt.savefig("./my_test/" + TASK_NAME + "/VGG16_accuracy.png", dpi=300, bbox_inches='tight')
plt.close()  # 关闭图像，释放资源
...
ax1.set_title("VGG16lite Absolute Accuracy Loss (Default VGG16lite vs LC + dLoRA VGG16lite / LC VGG16lite)")
plt.plot(np.abs(np.subtract(np.array(full_accuracy), 
                     np.array(restored_accuracy))), label = "LC + dLoRA VGG16lite")
plt.plot(np.abs(np.subtract(np.array(full_accuracy), 
                     np.array(lc_accuracy))), label = "LC VGG16lite")
plt.legend()
...
ax2.set_title("VGG16lite Absolute Restoration Accuracy Loss (LC + dLoRA VGG16lite & LC VGG16lite)")
plt.plot(np.abs(np.subtract(np.array(restored_accuracy), 
                     np.array(decomposed_full_accuracy))), label = "LC + dLoRA VGG16lite")
plt.plot(np.abs(np.subtract(np.array(full_accuracy), 
                     np.array(lc_accuracy))), label = "LC VGG16lite")
plt.legend()
plt.ylim(0, 0.5)
...
plt.savefig("./my_test/" + TASK_NAME + "/VGG16lite_loss.png", dpi=300, bbox_inches='tight')
plt.close()  # 关闭图像，释放资源
